...
DB_TOKEN, FIND_TOKEN, INSERT_TOKEN, UPDATE_TOKEN,  STRING_TOKEN,NUM_TOKEN,  TRUE_TOKEN,
 ...
 IN_TOKEN, NIN_TOKEN, AND_TOKEN, OR_TOKEN, NOR_TOKEN, FIN_TOKEN , REMOVE_TOKEN
             } CODES_LEX;
...
,QAUX5_ERR , DOUBLE_ERR , REMOVE_ERR
             }Erreurs;
...
//Follow (queryaux) = {'}'}
             switch (sym_cour.code){
                 case STRING_TOKEN : qu=creer_Query(autres);
                                    char * tst = (char*)malloc(20*sizeof(char));
                                    strcpy(tst,sym_cour.nom);
                                    Query_aux* qux = creerQueryAux(tst);
                                    ajouterQuery(qu,qux);
    ...
    switch(sym_cour.code){
        ...
        case MO_TOKEN: sym_suiv();
                switch(sym_cour.code){
                    ...
                    case STRING_TOKEN :  /*sym_suiv();A VERIFIER*/ ELEMENTS();if(liste_membres!=NULL)strcat(liste_membres,sym_cour.nom);Test_Symbole(MF_TOKEN,MF_ERR);break;
