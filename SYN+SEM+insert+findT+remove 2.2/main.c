...
DB_TOKEN, FIND_TOKEN, INSERT_TOKEN, UPDATE_TOKEN,  STRING_TOKEN,NUM_TOKEN,  TRUE_TOKEN,
 ...
 IN_TOKEN, NIN_TOKEN, AND_TOKEN, OR_TOKEN, NOR_TOKEN, FIN_TOKEN , REMOVE_TOKEN, GLL_TOKEN, USE_TOKEN , DROP_TOKEN , SHOW_TOKEN ,
  DBS_TOKEN, COLLECTIONS_TOKEN, DROPDATABASE_TOKEN,CREATECOLLECTION_TOKEN
             } CODES_LEX;
...
{OR_TOKEN,"$or"},{NOR_TOKEN,"$nor"},{GLL_TOKEN,"\""}, {SHOW_TOKEN,"show"} , {COLLECTIONS_TOKEN,"collections"} , {DBS_TOKEN,"dbs"} , {DROPDATABASE_TOKEN,"dropDatabase"},
{CREATECOLLECTION_TOKEN,"createCollection"}
};
...
switch(COD_ERR){
    ...
    case USE_ERR : printf("Erreur : Requete invalide \n");break;
    case DROP_ERR : printf("Erreur : Requete invalide \n");break;
...
void cmp_mot(){
 ...
 else       if ( !strcmp(sym_cour.nom,"use") ) { sym_cour.code=USE_TOKEN;}

 else       if ( !strcmp(sym_cour.nom,"show") ) { sym_cour.code=SHOW_TOKEN;}

 else       if ( !strcmp(sym_cour.nom,"dbs") ) { sym_cour.code=DBS_TOKEN;}

 else       if ( !strcmp(sym_cour.nom,"collections") ) { sym_cour.code=COLLECTIONS_TOKEN;}
 else       if ( !strcmp(sym_cour.nom,"drop") ) { sym_cour.code=DROP_TOKEN;}
 else       if ( !strcmp(sym_cour.nom,"dropDatabase") ) { sym_cour.code=DROPDATABASE_TOKEN;}
  else       if ( !strcmp(sym_cour.nom,"createCollection") ) { sym_cour.code=CREATECOLLECTION_TOKEN;}
...
switch (sym_cour.code){
    ...
    case EXISTS_TOKEN : if(qu!=NULL){
                                    qu->qAux->Op=(Operation*)malloc(sizeof(Operation));
                                    qu->qAux->Op->argList=(Argument*)malloc(sizeof(Argument));
                                    qu->qAux->Op->tOpComp=exists;
                                    qu->qAux->Op->suivant=NULL;
                                    qu->qAux->Op->argList->suivant=NULL;
                                }sym_suiv(); Test_Symbole(DEUXP_TOKEN,DEUXP_ERR);
         switch (sym_cour.code){
             case TRUE_TOKEN : qu->qAux->Op->argList->tArg=true_; qu->qAux->Op->argList->argValue=sym_cour.nom;sym_suiv();break;
             case FALSE_TOKEN : qu->qAux->Op->argList->tArg=false_;qu->qAux->Op->argList->argValue=sym_cour.nom;sym_suiv(); break;
...
char * chemin = NULL;
chemin = (char*)malloc(1024*sizeof(char));
strcpy(chemin,"");
strcat(chemin,che2);
strcat(chemin,"/");
...
interpreter_update(chemin,qu,ER);
}
...
strcat(chemin,collection);
    QUERY();
    interpreter_remove(chemin,qu,ER);
}
void LISTE_QUERY(){
...
void instaux(){
    switch(sym_cour.code){
        ...
        case USE_TOKEN : insts (); break;
        case SHOW_TOKEN : insts (); break;
...
db = (char*)malloc(1024*sizeof(char));
   che = (char*)malloc(1024*sizeof(char));
   che3 = (char*)malloc(1024*sizeof(char));
   arg = (char*)malloc(1024*sizeof(char));
   strcpy(che,"C://DBG//");
   strcpy(che3,"C://DBG//");

switch(sym_cour.code){
 case DB_TOKEN : Test_Symbole(DB_TOKEN,DB_ERR);
Test_Symbole(POINT_TOKEN,POINT_ERR);
strcpy(collection,sym_cour.nom);
strcat(collection,".txt");
            switch (sym_cour.code){
                  case STRING_TOKEN : sym_suiv(); Test_Symbole(POINT_TOKEN,POINT_ERR); {
                       switch(sym_cour.code){
                           case DROP_TOKEN : sym_suiv (); Test_Symbole(PARO_TOKEN,PARO_ERR);interpreter_drop();break;
                  ...
                  case CREATECOLLECTION_TOKEN : sym_suiv();Test_Symbole(PARO_TOKEN,PARO_ERR);Test_Symbole(GLL_TOKEN,GLL_ERR);
                  strcat(che4,sym_cour.nom);strcat(che4,".txt");
                  Test_Symbole(STRING_TOKEN,STRING_ERR);Test_Symbole(GLL_TOKEN,GLL_ERR);interpreter_createcol();break;
                  case DROPDATABASE_TOKEN : sym_suiv();Test_Symbole(PARO_TOKEN,PARO_ERR);remove_directory(che4);break;
                       default :  Error(INST_ERR) ; break;
                            }
                    Test_Symbole(PARF_TOKEN,PARF_ERR);break;
case USE_TOKEN : sym_suiv();
                strcpy(db,sym_cour.nom);strcat(che,db);strcat(che3,db);strcat(che3,"/");che4 = (char*)malloc(1024*sizeof(char));strcpy(che4,"");strcat(che4,che3); interpreter_use();
                che2 = (char*)malloc(1024*sizeof(char));
                strcpy(che2,che);
                Test_Symbole(STRING_TOKEN,STRING_ERR);break;
case SHOW_TOKEN : sym_suiv();
     switch(sym_cour.code){
        case COLLECTIONS_TOKEN : Test_Symbole(COLLECTIONS_TOKEN,COLLECTIONS_ERR);interpreter_show_coll();break;//interpreter_show_coll();
        case DBS_TOKEN : Test_Symbole(DBS_TOKEN,DBS_ERR);interpreter_show_db();break;//interpreter_show_db();
        default :  Error(SHOW_ERR) ; break;
}
...
void interpreter_use(){
if (ER==0){
 CreateDirectory (che, NULL);
}
}
void interpreter_createcol(){
if (ER==0){
FILE*fichier=fopen(che4,"a+");
fclose(fichier);
...
void interpreter_show_db(){
    if(ER==0){
show_piw("C:/DBG/");
}
}
void interpreter_show_coll(){
    if(ER==0){
       show_piw(che4);
}
}

void show_piw(char * chemin){
